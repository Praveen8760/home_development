...
window.addEventListener("load", function () {
    gsap.to("#loading-overlay", { opacity: 0, duration: 0.5, onComplete: () => document.getElementById("loading-overlay").style.display = "none" });

    gsap.registerPlugin(ScrollTrigger, ScrollToPlugin);

    // Navbar Animation
    gsap.to("#navbar", { opacity: 1, y: 0, duration: 1.5, ease: "power2.out", 
        onComplete: function () {
            animateHeroSection();
        } 
    });
    ...
    gsap.from("#animated-footer", {
        ...
        duration: 2.5,
        ease: "power3.out",
        scrollTrigger: { trigger: "#animated-footer", start: "top 90%" }
    });
